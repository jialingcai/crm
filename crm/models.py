...
class UserProfile(models.Model):
    ...
    department = models.ForeignKey('Department', default=None, blank=True, null=True,verbose_name="所属部门", on_delete=models.CASCADE)
    mobile = models.CharField(verbose_name='手机号', max_length=32, default=None, blank=True, null=True,)
    memo = models.TextField(verbose_name='备注', blank=True, null=True, default=None)
    date_joined = models.DateTimeField(auto_now_add=True,verbose_name="入职时间")
    is_active = models.BooleanField(default=True,verbose_name="活跃度")
...
class Customer(models.Model):
    ...
    def show_classes(self):
        return " |".join(str(i) for i in self.class_list.all())   # 列表生成式 存放一个个对象的字符串
    ...
    def show_state(self):
        ...
        return mark_safe('<span style="background-color:{};color:white; padding:3px;">{}</span>'.format(color_dict.get(self.status), self.get_status_display()))
